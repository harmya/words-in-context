...
class DAN(torch.nn.Module):
    def __init__(self):
        ...
        self.third_hidden_layer = torch.nn.Linear(32, 32)
        self.output_layer = torch.nn.Linear(32, 1)

    def forward(self, x):
        return torch.sigmoid(self.output_layer(torch.relu(self.third_hidden_layer(torch.relu(self.second_hidden_layer(torch.relu(self.first_layer(x))))))))
